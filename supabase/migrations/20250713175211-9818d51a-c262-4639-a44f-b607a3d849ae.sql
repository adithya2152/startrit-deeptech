-- Add user_id column to profiles table for Gmail prefix-based IDs
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS user_id TEXT UNIQUE;

-- Create index for faster searches
CREATE INDEX IF NOT EXISTS idx_profiles_user_id ON public.profiles(user_id);

-- Update RLS policies to include user_id searches
...
-- Update the handle_new_user function to set user_id as Gmail prefix
...
BEGIN
  ...
  -- Check for existing user_id if it's a Gmail address
  IF gmail_prefix IS NOT NULL THEN
    -- Check if this prefix already exists
    IF EXISTS (SELECT 1 FROM public.profiles WHERE user_id = gmail_prefix) THEN
      RAISE EXCEPTION 'A user with this Gmail prefix already exists. Please use a different email address.';
    END IF;
  END IF;
  
  INSERT INTO public.profiles (user_id, first_name, last_name, user_id)
  VALUES (
    NEW.id, 
...
-- Update existing profiles without user_id (for existing users)
UPDATE public.profiles 
SET user_id = extract_gmail_prefix((
